...
//   import { K8sKind, K8sResourceKind, referenceForModel } from '@console/internal/module/k8s';
  ...
  export type TaskRunKind = { pipelineTaskName?: string };
  ...
  export interface PipelineResourceTask  {
    spec: {
  ...
  export const getResources = (data: PropPipelineData[]): Resource => {
    const resources: { kind: any; namespace: string; isList: boolean; prop: string; selector: { 'tekton.dev/pipeline': string; }; }[] = [];
    const propsReferenceForRuns: string[] = [];
    if (data && data.length > 0) {
      data.forEach((pipeline, i) => {
        //TODO:should request golang api by axios
        //TODO:fix
  ...
  export const getResourceModelFromTaskKind = (kind: string) =>
    kind === ClusterTaskModel.kind ? ClusterTaskModel : TaskModel;
  
  export const getResourceModelFromBindingKind = (kind: string) =>
    kind === ClusterTriggerBindingModel.kind ? ClusterTriggerBindingModel : TriggerBindingModel;
  
  export const getResourceModelFromTask = (task: PipelineTask) => {
    const {
      taskRef: { kind },
    } = task;
